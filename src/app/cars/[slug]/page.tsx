...
import CarMediaList from "@/cars/CarMediaList";
import { CAR_LIST, CAR_MEDIA_LIST } from "@/cars/data";
import { getCarTitle } from "@/cars/utils";

import WheelCard from "@/rims-tires/WheelCard";

export default function Page() {
  const params = useParams<{ slug: string }>();
  const carTitle = getCarTitle(params?.slug ?? "");

  const car = CAR_LIST[0];

  const carPrice = useMemo(
    () => currency(car.price_cts, { fromCents: true }).format(),
    [car.price_cts],
  );
  ...
  return (
    <Container className="m-auto gap-8 flex flex-col py-8">
      <ul className="inline-flex gap-2 items-center text-sm">
        ...
        <li className="text-sm">{carTitle}</li>
      </ul>

      <div className="flex gap-4">
        <div className="w-[785px] flex flex-col gap-16">
          <CarMediaList mediaList={CAR_MEDIA_LIST} />

          <div className="flex flex-col gap-2">
            <h1>{carTitle}</h1>
            <span className="font-bold text-2xl">{carPrice}</span>
            <p className="text-sm">
